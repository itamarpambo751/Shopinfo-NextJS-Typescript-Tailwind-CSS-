...
const Page:React.FC=()=>{
     ...
     const typeItems = typeof type === 'string' ? type : '';
      ...
      useEffect(() => {
        ...
        else setTypes(products?.filter((i)=>i.marca.toLowerCase().startsWith(filterProduct.toLocaleLowerCase())))
      },[filterProduct]); 

useEffect(() => {
   setTypes(products?.filter((i)=>i.name.toLowerCase().startsWith(searC.toLowerCase())|| i.name.toLowerCase().startsWith(searC.toLowerCase())))
  },[products]); 
